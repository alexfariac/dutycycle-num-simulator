        ...
        public static void NDTMetrics(ScheduleMethod schedule1, ScheduleMethod schedule2)
        {
            ...
            for (int startIndex1 = 0; startIndex1 < lcm; startIndex1++)
            {
                for(int startIndex2 = startIndex1; startIndex2 < lcm; startIndex2++)
                {
                    ...
                    discoveryTimeSum += (ulong)discoveryTime;
                    maxDiscoveryTime = Math.Max(maxDiscoveryTime, discoveryTime);
                    iterations++;
                }
            }

            double averageNdt = Math.Round((double)discoveryTimeSum / iterations, ROUND_PRECISION);
            Console.WriteLine($"S1: {schedule1.Name}; S2: {schedule2.Name}; gcf: {gcf}; DC1: {Math.Round(schedule1.DutyCyclePerc, ROUND_PRECISION)}; DC2: {Math.Round(schedule2.DutyCyclePerc, ROUND_PRECISION)}; AVG-NDT: {averageNdt}; MAX-NDT: {maxDiscoveryTime}");
        }
